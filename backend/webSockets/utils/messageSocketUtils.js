...
static async softDeleteMessage(messageId) {
   try {
      const result = await ServiceFactory.getMessageService.softDeleteMessage(messageId);
      const message = await ServiceFactory.getMessageService.getDocumentById(messageId);
      return message;
   } catch (error) {
      return null;
...
static async getMessageRecipients(messageId) {
   try {
      ...
      // Get phone numbers
      const users = await ServiceFactory.getUserService.getDocumentsByCustomFilters({
