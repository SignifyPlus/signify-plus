...
class FirebaseManager {
   ...
   async #setFirebaseCreds(base64Creds) {
      this.#firebaseCreds = new FirebaseAdmin(
         JSON.parse(await CommonUtils.decodeFromBase64(base64Creds)),
      );
      LoggerFactory.getApplicationLogger.info(
         `Firebase Credentials are all set!`,
      );
   }
   ...
   async connectToFireBase(base64Creds) {
      try {
         ...
         LoggerFactory.getApplicationLogger.error(
            `Firebase Exception: ${exception}`,
         );
         throw exception;
      }
   }

   async createUserAccountByPhoneNumber(phoneNumber) {
      //E.164 format
      try {
         ...
         LoggerFactory.getApplicationLogger.error(
            `Account Exception: ! ${exception.code}`,
         );
      }
   }

   async getUserAccountByPhoneNumber(phoneNumber) {
      //E.164 format
      try {
         const existingAccount = await this.#adminSdk
            .auth()
